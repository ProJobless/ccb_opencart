<?php
class Modeltesoreriacuentagastos extends Model {
	...
	public function getBalanceCuentaGastos() {
		$query = $this->db->query("SELECT SUM(amount) AS balanceTotal FROM " . DB_PREFIX . "customer_transaction where tipo_cuenta = '1'");
	...
	*/
	public function getListaSaldosCuentaGastos() {
		$query = $this->db->query("SELECT c.firstname as firstname , c.lastname as lastname, c.customer_id as id, SUM(amount) AS saldo FROM " . DB_PREFIX . "customer_transaction t, ". DB_PREFIX . "customer c where t.customer_id = c.customer_id and t.tipo_cuenta = '1' group by c.customer_id");

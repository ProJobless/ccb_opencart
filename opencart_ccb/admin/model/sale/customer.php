...
class ModelSaleCustomer extends Model {
	...
	public function addTransaction($customer_id, $description = '', $amount = '', $tipo_cuenta = '', $order_id = 0) {
		$customer_info = $this->getCustomer($customer_id);
		
		$amount = str_replace(",", ".", $amount);
		
		if ($customer_info) { 
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_transaction SET customer_id = '" . (int)$customer_id . "', order_id = '" . (int)$order_id . "', description = '" . $this->db->escape($description) . "', amount = '" . (float)$amount . "', date_added = NOW()". ", tipo_cuenta = '" . (int)$tipo_cuenta."'");
	...
	public function getTransactionPedidosTotal($customer_id) {
		$query = $this->db->query("SELECT SUM(amount) AS total FROM " . DB_PREFIX . "customer_transaction WHERE customer_id = '" . (int)$customer_id . "' and tipo_cuenta='0'");
	
		return $query->row['total'];
	}
			
	public function getTransactionGastosTotal($customer_id) {
		$query = $this->db->query("SELECT SUM(amount) AS total FROM " . DB_PREFIX . "customer_transaction WHERE customer_id = '" . (int)$customer_id . "' and tipo_cuenta='1'");

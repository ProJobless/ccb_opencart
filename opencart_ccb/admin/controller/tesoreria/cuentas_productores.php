<?php
class Controllertesoreriacuentasproductores extends Controller { 
	private $error = array();

	public function index() {
		$this->load->language('tesoreria/cuentas_productores');

		$this->document->setTitle($this->language->get('heading_title'));
		 
		$this->load->model('tesoreria/cuentas_productores');
    ...
    foreach ($results as $result) {
		...
		$this->template = 'tesoreria/cuenta_pedidos.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm() {
        ...
        /*INICIO DE CUENTAS PRODUCTORES*/
	private function getHome() {
		
		//Configuro la url de enlaces
		$url = '';
		...
		$this->data['breadcrumbs'] = array();
 		...
 		$this->data['breadcrumbs'][] = array(
     		                'text'      => $this->language->get('heading_title'),
				'href'      => $this->url->link('tesoreria/cuentas_productores', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    		                'separator' => ' :: '
 		);
 		
		//Declaro si hay success
		if (isset($this->session->data['success'])) {
			$this->data['success'] = $this->session->data['success'];
		...
		//Declaro si hay error warning				
 		if (isset($this->error['warning'])) {
			...
			$this->data['error_warning'] = '';
                ...
                //Cojo encabezados de las properties de multidioma
		$this->data['productor'] = $this->language->get('productor');
		$this->data['numero_factura'] = $this->language->get('numero_factura');
		$this->data['importe_factura'] = $this->language->get('importe_factura');		
		$this->data['fecha_factura'] = $this->language->get('fecha_factura');		
		$this->data['fecha_comentario'] = $this->language->get('fecha_comentario');		
		$this->data['comentario'] = $this->language->get('comentario');		

                $currency_info = $this->model_localisation_currency->getCurrencyByCode($this->config->get('config_currency'));
		$currency_code = $currency_info['code'];
		$currency_value = $currency_info['value'];

		//Saco la informacion de las facturas de los proveeedores
		$results = $this->model_tesoreria_cuentas_productores->getAllFacturasProductores();
		
                foreach ($results as $result) {
			$manufacturer_id = $result['manufacturer_id'];
			$action = array(
				'text' => 'Ver movimientos',
				'href' => $this->url->link('sale/manufacturer/update', 'token=' . $this->session->data['token'] . '&manufacturer_id =' . $manufacturer_id . $url, 'SSL')
			);
						
			$this->data['facturas'][] = array(
				'manufacturer_name' 	=> $result['manufacturer_name'],
				'numero_factura'        => $result['numero_factura'],
				'manufacturer_id'  	=> $manufacturer_id ,
				'importe_factura'    	=> $this->currency->format($result['importe_factura'], $currency_code, $currency_value),
                                'fecha_factura'         => date($this->language->get('date_format_short'), strtotime($result['fecha_factura'])),
				'action'                => $action
			);
		}		
                 
		//Saco la informacion de los comentarios de los proveeedores
		$results = $this->model_tesoreria_cuentas_productores->getAllComentariosProductores();
			...
			$this->data['comentarios'][] = array(
				'manufacturer_name' 	=> $result2['manufacturer_name'],
				'comentario'            => $result2['comentario'],
				'manufacturer_id'  	=> $manufacturer_id ,
                                'fecha_comentario'      => date($this->language->get('date_format_short'), strtotime($result2['fecha_comentario'])),
				'action'                => $action
			);
		}		

		//Configuro los datos que se pintan
		$this->data['heading_title'] = $this->language->get('heading_title');
		
		//defino la pantalla de vista de los resultados
		$this->template = 'tesoreria/cuentas_productores.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);

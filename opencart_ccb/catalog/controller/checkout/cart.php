...
class ControllerCheckoutCart extends Controller {
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				...
				foreach ($results as $result) {
					if ($result['code']!='credit' && $this->config->get($result['code'] . '_status')) {
	...
	public function add() {
		...
		if ($product_info) {			
			...
			//$json['success'] = "JES 11111-->".$product_info['minimum'] . " , quantity: ".$quantity;
			//$this->response->setOutput(json_encode($json));	
			//return;
			
			//Validamos cantidad minima
			if ($product_info['minimum'] && $quantity < $product_info['minimum']) {
				$json['error']['minimum'] = sprintf("La cantidad minima es de ".$product_info['minimum']);
			}
